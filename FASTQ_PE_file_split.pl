...
my $usage = "
Given a fastq file with paired reads, this script will split it to a 
number of files. Each file will contain given number of sequences. 
Generated files have the same name as the given file with numbered 
suffix .file0.fa .file1.fa ... etc. This script make sure paired reads
...
<-n> = minimum number of reads in each file, 3000000 for velvet assembly
...
$total_seq = $total_seq/8; # because this is for PE sequence, each pair has 8 lines.
print "total pair of seq = $total_seq\n";

# calculate how many sequences in each file
my $size = int ($total_seq/$numFile) + 1;
print "total paire of seq $total_seq, # seq pair per file:  $size\n";
...
while (my $line = <IN>){
	...
	$line = <IN>; 
	print OUT "$line"; # first line of second read
	$line = <IN>; 
	print OUT "$line"; # 2nd line of second read 
	$line = <IN>; 
	print OUT "$line"; # 3rd line of second read
	$line = <IN>; 
	print OUT "$line"; # 4th line of second read
